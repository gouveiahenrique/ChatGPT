...
import com.ciandt.flow.icons.ChatGPTIcons;
import com.ciandt.flow.settings.OpenAISettingsState;
import com.ciandt.flow.util.ImgUtils;
    ...
    public MessageComponent(String content, boolean me) {
        ...
        copyAction.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                String copiedCode = me ? question : answer;
                ClipboardUtil.setStr(copiedCode);
                String regex = "```(.*?)";
                Pattern pattern = Pattern.compile(regex);
                Matcher matcher = pattern.matcher(copiedCode);
                String language = "unknown";

                if (matcher.find()) {
                    System.out.println("OLHA O GRUPO" + matcher.group(1));
